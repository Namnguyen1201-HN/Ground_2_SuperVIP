...
public class OrderDAO extends DataBaseContext {
    ...
    /**
     * Get revenue reports from Orders table with customer and user information
     */
    public List<RevenueReportDTO> getRevenueReports(
            Date fromDate, Date toDate, Integer branchId, 
            Integer employeeId, String paymentMethod) {
        ...
        sql.append("    ISNULL(c.FullName, N'Khách lẻ') AS CustomerName, ");
        sql.append("    o.PaymentMethod, ");
        sql.append("    o.CreatedAt, ");
        sql.append("    o.BranchID, ");
        sql.append("    ISNULL(b.BranchName, N'Không xác định') AS BranchName, ");
        sql.append("    ISNULL(u.FullName, N'Không xác định') AS CreatedByName, ");
        sql.append("    o.GrandTotal ");
        sql.append("FROM Orders o ");
        sql.append("LEFT JOIN Customers c ON o.CustomerID = c.CustomerID ");
        sql.append("LEFT JOIN Branches b ON o.BranchID = b.BranchID ");
        ...
        if (branchId != null && branchId > 0) {
            sql.append("AND o.BranchID = ? ");
            params.add(branchId);
        }
        
        // Employee filter
        if (employeeId != null && employeeId > 0) {
            sql.append("AND o.CreatedBy = ? ");
        ...
        sql.append("ORDER BY o.CreatedAt DESC, o.OrderID DESC");
        ...
        try (PreparedStatement ps = connection.prepareStatement(sql.toString())) {
            ...
            while (rs.next()) {
                ...
                dto.setGrandTotal(rs.getBigDecimal("GrandTotal"));
    ...
    public BigDecimal getTotalRevenue(
        ...
        StringBuilder sql = new StringBuilder();
        sql.append("SELECT ISNULL(SUM(o.GrandTotal), 0) AS TotalAmount ");
        sql.append("FROM Orders o ");
        sql.append("WHERE 1=1 ");
