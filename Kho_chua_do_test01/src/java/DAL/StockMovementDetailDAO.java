...
public class StockMovementDetailDAO extends DataBaseContext {
        ...
        try (PreparedStatement ps = connection.prepareStatement(query)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    ...
                    detail.setMovementDetailId(detailID);
    ...
    public List<StockMovementDetail> getRawDetailsByMovementID(String dbName, int movementID) throws SQLException {
        ...
        String sql = "SELECT d.MovementDetailID, d.MovementID, d.ProductDetailID, d.Quantity, d.QuantityScanned, "
                + "pd.ProductCode, p.ProductName, pd.ProductID "
                + // Thêm pd.ProductID
                "FROM StockMovementDetail d "
                + "JOIN ProductDetails pd ON d.ProductDetailID = pd.ProductDetailID "
                + "JOIN Products p ON pd.ProductID = p.ProductID "
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    ...
                    if (rs.getInt("SerialCount") > 0) {
                        detail.setSerials(getSerialsByMovementDetail(dbName, detail.getMovementDetailId()));
                    } else {
        ...
        try (
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    ...
                    List<ProductDetailSerialNumber> serials = getSerialsByDetailForView(dbName, detail.getMovementDetailId());
