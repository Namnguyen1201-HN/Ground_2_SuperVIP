...
public class StockMovementDAO extends DataBaseContext {
    ...
    private StockMovementsRequest mapHeader(ResultSet rs) throws SQLException {
        ...
        int v;
        v = rs.getInt("FromSupplierID");  if (!rs.wasNull()) r.setFromSupplierId(v);
        v = rs.getInt("FromBranchID");    if (!rs.wasNull()) r.setFromBranchId(v);
        v = rs.getInt("FromWarehouseID"); if (!rs.wasNull()) r.setFromWarehouseId(v);
        v = rs.getInt("ToBranchID");      if (!rs.wasNull()) r.setToBranchId(v);
        v = rs.getInt("ToWarehouseID");   if (!rs.wasNull()) r.setToWarehouseId(v);
    ...
    public PagedMoves list(String type, Integer branchId, Integer warehouseId,
        ...
        if (branchId != null) {
            ...
            ps.add(branchId); ps.add(branchId);
        }
        if (warehouseId != null) {
            where.append(" AND (FromWarehouseID = ? OR ToWarehouseID = ?)");
            ps.add(warehouseId); ps.add(warehouseId);
        }
        ...
        try (PreparedStatement psmt = connection.prepareStatement(sql)) {
            int idx = 1;
            for (Object p : ps) psmt.setObject(idx++, p);
        ...
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    ...
                    if (!rs.wasNull()) d.setQuantityScanned(qs);
                    list.add(d);
                }
            }
        } catch (SQLException e) { e.printStackTrace(); }
    ...
    public boolean updateHeaderFull(
        ...
        String sql = "UPDATE StockMovementsRequest " +
                ...
                "WHERE MovementID=?";
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            if (fromSupplierId  != null) ps.setInt(1, fromSupplierId);  else ps.setNull(1,  Types.INTEGER);
            if (fromBranchId    != null) ps.setInt(2, fromBranchId);    else ps.setNull(2,  Types.INTEGER);
            if (fromWarehouseId != null) ps.setInt(3, fromWarehouseId); else ps.setNull(3,  Types.INTEGER);
            if (toBranchId      != null) ps.setInt(4, toBranchId);      else ps.setNull(4,  Types.INTEGER);
            if (toWarehouseId   != null) ps.setInt(5, toWarehouseId);   else ps.setNull(5,  Types.INTEGER);
