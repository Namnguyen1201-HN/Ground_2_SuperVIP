...
public class UserDAO extends DataBaseContext {
    ...
    public List<User> getFilteredUsers(String branch, String role, String status, String keyword) {
        List<User> list = new ArrayList<>();
        StringBuilder sql = new StringBuilder(
                "SELECT u.UserID, u.FullName, u.Email, u.Phone, u.IdentificationID, "
                + "b.BranchName, r.RoleName, u.IsActive "
                + "FROM Users u "
                + "LEFT JOIN Branches b ON u.BranchID = b.BranchID "
        ...
        if (branch != null && !"all".equalsIgnoreCase(branch)) {
            sql.append(" AND b.BranchName = ?");
        }
        if (role != null && !"None".equalsIgnoreCase(role)) {
