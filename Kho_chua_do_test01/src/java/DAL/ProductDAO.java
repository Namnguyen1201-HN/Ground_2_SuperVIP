...
public class ProductDAO extends DataBaseContext {
    ...
    public List<Product> findProductsWithThresholdByCategoryNames(List<String> categoryNames, String keyword, String stock, int threshold) {
        ...
        try (PreparedStatement ps = connection.prepareStatement(sql.toString())) {
            int idx = 1;
            for (Object o : params) {
                if (o instanceof String s) ps.setString(idx++, s);
                else if (o instanceof Integer i) ps.setInt(idx++, i);
    ...
    private Product mapRowToProduct(ResultSet rs) throws SQLException {
        ...
        int brandId = rs.getInt("BrandID");     if (!rs.wasNull()) p.setBrandId(brandId);
        int catId   = rs.getInt("CategoryID");  if (!rs.wasNull()) p.setCategoryId(catId);
        int supId   = rs.getInt("SupplierID");  if (!rs.wasNull()) p.setSupplierId(supId);
    ...
    public static class ProductWithStock {
        ...
        public ProductWithStock(Product product, Integer totalQty) { this.product = product; this.totalQty = totalQty; }
